...
/**
  ...
  @notice To be implemented by ClaimPresale.
 */
abstract contract IFFundable is Ownable, ReentrancyGuard {
    ...
    // tracks whether payment token has been cashed
    bool public hasCashedPaymentToken;
    // tracks whether sale token has been cashed
    bool public hasCashedSaleToken;
    ...
    constructor(
        ...
        // when salePrice != 0, paymentToken shouldn't be 0
    ...
    modifier onlyAfterSale {
        require(block.timestamp > endTime, "can't withdraw before sale has ended");
        _;
    }
    ...
    function cashAllPaymentToken() external onlyCasherOrOwner onlyAfterSale{
        // prevent repeat cash
        require(!hasCashedPaymentToken, 'already cashed payment token');

        hasCashedPaymentToken = true;
        ...
        emit CashPaymentToken(_msgSender(), paymentTokenBal);
    }

    // Function for funder to cash in unsold sale token
    function cashSaleToken() external onlyCasherOrOwner onlyAfterSale {
        // prevent repeat cash
        require(!hasCashedSaleToken, 'already cashed');

        hasCashedSaleToken = true;

        // get amount of sale token on contract
        uint256 saleTokenBal = saleToken.balanceOf(address(this));

        // transfer unsold
        saleToken.safeTransfer(_msgSender(), saleTokenBal);

        emit CashSaleToken(_msgSender(), saleTokenBal);
    }
    ...
    function cashPaymentToken(uint256 amount) external onlyCasherOrOwner {
        ...
        emit CashPaymentToken(_msgSender(), amount);
    }
...
/**
  @dev Abstract contract containing sale logics.
        To be implemented by ClaimPresale.
...
abstract contract IFPurchasable is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(
            ...
            'paymentToken should not be 0 when salePrice is not 0'
        );
        // saleToken shouldn't be the same as paymentToken
        require(_saleToken != _paymentToken, 'saleToken = paymentToken');
    ...
    function _purchase(uint256 paymentAmount, uint256 remaining) virtual internal nonReentrant {
        ...
        // calculate sale tokens to be transfered
        uint256 saleTokenTransfered = (paymentAmount) / salePrice;

        // transfer tokens bought
        saleToken.safeTransfer(_msgSender(), saleTokenTransfered);
...
// File contracts/ClaimPresale.sol
...
/**
  @dev Vanilla Sale contract compositing funding, and purchase functions.
  @notice Features:
  @notice 1. Funder related actions like fund and cash
  @notice 2. Regular purchase

 */
contract ClaimPresale is IFPurchasable, IFFundable {

    // tracks amount of tokens purchased by each address
    mapping(address => uint256) public totalPurchased;
    // flag to enable integer sale
    bool public isIntegerSale = true;
    // allocation for each wallet
    // note that any user can get publicAllocation
    uint256 public publicAllocation = 0;
    // terms of sale for the contract
    string private termsOfSale;
    // tracks users who have signed the terms
    mapping(address => uint256) private signedTermsOfSale;

    // --- EVENTS
    // track terms of sale signed
    event SignTermsOfSale(address indexed signer);
    ...
    constructor(
        ...
        IFPurchasable(_paymentToken, _saleToken, _salePrice)
        IFFundable(_paymentToken, _saleToken, _startTime, _endTime)
    {
    }
    ...
    function setTermsOfSale(string memory _termsOfSale) public onlyOwner onlyBeforeSale {
        termsOfSale = _termsOfSale;
    }

    // --- PURCHASE ACTIONS

    // purchase with code function
    function purchaseWithCode(
        uint256 paymentAmount,
        string calldata code
    ) public onlyDuringSale {
        uint256 allocation = publicAllocation;
        uint256 remaining = _getMaxPayment(_msgSender(), allocation);
    ...
    function purchase(
        uint256 paymentAmount
    ) public override onlyDuringSale {
        uint256 allocation = publicAllocation;
        uint256 remaining = _getMaxPayment(_msgSender(), allocation);
        _purchase(paymentAmount, remaining);
    }
    ...
    function _purchaseWithCode(uint256 paymentAmount, uint256 remaining, string memory code) override internal {
        if (isIntegerSale) {
            require(isIntegerPayment(paymentAmount), 'can only buy integer amount of sale tokens');
        }
        require(getSignedTermsOfSale(_msgSender()) > 0, 'only signed users can initiate purchase');
    ...
    function isIntegerPayment(uint256 paymentAmount) public view returns (bool) {
        return (paymentAmount) % salePrice == 0;
    }
    ...
    // check how much can a wallet buy based on per wallet limit
    // @dev get max payment from allocation 
    function _getMaxPayment(address user, uint256 allocation) virtual internal returns (uint256) {
        // get the maximum total payment for a user
        uint256 max = (salePrice * allocation);
    ...
    function getTermsOfSale() public view returns (string memory) {
        return termsOfSale;
    }

    // only sender can sign it
    function signTermsOfSale() public onlyDuringSale{
        require(signedTermsOfSale[_msgSender()] == 0, 'terms of sale already signed');
        signedTermsOfSale[_msgSender()] = block.timestamp;
        emit SignTermsOfSale(_msgSender());
    }

    // return epoch timestamp when the terms of sale has been signed
    function getSignedTermsOfSale(address user) public view returns (uint256){
        return signedTermsOfSale[user];
    }
